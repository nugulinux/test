...
echo "Get file list"
FILES=$(curl -s -H "Authorization: token ${GITHUB_TOKEN}" -X GET https://api.github.com/repos/${REPOSLUG}/releases/${ID}/assets | jq '.[].browser_download_url' -r)
for item in $FILES
do
    if [[ $item != *.deb ]]; then
        echo "- ignore: ${item}"
...
<h3>Bionic (18.04)</h3>
<pre>
    echo "deb [trusted=yes] https://nugulinux.github.io/test/ubuntu/ bionic main" > /etc/apt/sources.list.d/nugudev.list
    apt-get update
</pre>

<h3>Xenial (16.04)</h3>
<pre>
    echo "deb [trusted=yes] https://nugulinux.github.io/test/ubuntu/ xenial main" > /etc/apt/sources.list.d/nugudev.list
    apt-get update
</pre>
